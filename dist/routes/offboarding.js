...
import { calculateOffboardingSummary } from '../services/offboarding.js';
export const offboardingRouter = Router();
offboardingRouter.get('/summary', (req, res) => {
    const { employeeId, exitDate, reason } = req.query;
...
function normaliseReason(value) {
    ...
    if (lower === 'termination')
        return 'termination';
    if (lower === 'resignation')
        return 'resignation';
    if (lower === 'end-of-contract' || lower === 'end_of_contract' || lower === 'endofcontract')
