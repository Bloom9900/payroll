import { addEmployee, listEmployees } from './employeeRegistry.js';
function eurosToCents(value) {
    return Math.round(value * 100);
}
function determineFirstPayrollMonth(startDate) {
    const start = new Date(startDate + 'T00:00:00Z');
    return `${start.getUTCFullYear()}-${(start.getUTCMonth() + 1).toString().padStart(2, '0')}`;
}
function determinePayrollCutoff(startDate) {
    const start = new Date(startDate + 'T00:00:00Z');
    const cutoff = new Date(Date.UTC(start.getUTCFullYear(), start.getUTCMonth(), 25));
    return cutoff.toISOString().slice(0, 10);
}
function buildNewEmployeeRecord(input) {
    return {
...
function defaultOnboardingTasks(input) {
    const tasks = [
...
export function createJoiner(input) {
    const required = ['firstName', 'lastName', 'email', 'startDate', 'iban', 'annualSalaryEuros', 'hoursPerWeek', 'department', 'role', 'location'];
...
export function getUpcomingJoiners(month) {
...
export function fetchWorkdayJoinerStub(workdayWorkerId) {
    return {
        firstName: 'Workday',
