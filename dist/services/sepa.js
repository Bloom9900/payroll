...
export function buildPain001(debtor, payments) {
    if (!validateIBAN(debtor.iban))
        throw new Error('Invalid company IBAN');
    payments.forEach(p => { if (!validateIBAN(p.iban))
        throw new Error('Invalid employee IBAN for ' + p.name); });
