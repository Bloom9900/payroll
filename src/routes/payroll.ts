...
import type { PayrollRunRecord } from '../services/payrollRunLedger.js'
...
function centsToEuroString (value: number): string {
...
function calculationsForPeriod (month: string, paymentDate?: string, dueDate?: string) {
...
function formatRun (run: PayrollRunRecord) {
  return {
    id: run.id,
    period: run.period,
    status: run.status,
...
payroll.patch('/runs/:id/status', (req, res) => {
  ...
  const updated = updatePayrollRunStatus(id, status as any, { changedBy, note })
  if (!updated) {
