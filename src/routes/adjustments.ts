...
import type { AdjustmentType } from '../services/adjustmentsLedger.js'
...
adjustmentsRouter.post('/', (req, res) => {
  ...
  const validTypes: AdjustmentType[] = ['allowance', 'deduction', 'reimbursement', 'retro']
  if (!validTypes.includes(typeInput as AdjustmentType)) {
  ...
  const record = addAdjustment({
    ...
    type: typeInput as AdjustmentType,
    amountCents,
    taxable,
    recurring,
    effectiveMonth,
